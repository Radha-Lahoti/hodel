...
We agree this is an important point. We conducted an additional ablation on the 1D system by varying the number of training trajectories:
...
In addition, we evaluated a highly nonlinear 1D system (slinky), where the learned energy dominates the prior. The resulting force–displacement curves (MLP vs ICNN) compared to ground truth are shown here:  
